...
class ModbusRuntimeManager:
    ...
    def _normalize_reference(address: int, register_type: str, zero_based: bool) -> tuple[str, int]:
        """
        Normalize simulator addressing to Kepware-compatible decimal references.
        Supports 5-digit and 6-digit equivalence:
        - 41, 401, 4001, 40001, 400001 => table 4, item 1
        - 30001/300001 => input register item 1
        - 100001 => input coil item 1
        - 000001 (entered as 1) => output coil item 1
        """
        raw = int(address)
        if raw < 0:
            raise ValueError("Modbus address must be >= 0")

        configured = (register_type or "").strip().lower() or "holding"
        table_to_digit = {"coil": "0", "discrete": "1", "input": "3", "holding": "4"}
        digit_to_table = {v: k for k, v in table_to_digit.items()}
        configured_digit = table_to_digit.get(configured, "4")

        # Explicit raw-offset escape hatch.
        if raw == 0:
            return configured, 0

        token = str(raw).strip()
        inferred_type = configured
        item = raw

        # 5/6-digit references use explicit primary table digit (e.g. 40001, 300001).
        if len(token) >= 5 and token[0] in digit_to_table:
            inferred_type = digit_to_table[token[0]]
            item = int(token[1:])
        # Shorter explicit shorthand that matches the configured table (e.g. 41 for holding).
        elif len(token) >= 2 and token[0] in digit_to_table and token[0] == configured_digit:
            inferred_type = configured
            item = int(token[1:])
        # Ambiguous short form (e.g. "1"): treat as item number in the selected table.
        else:
            inferred_type = configured
            item = int(token)
